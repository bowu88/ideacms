...
class ApiController extends Common {
	...
	public function downAction() {
		...
		$file = isset($data['lpcms']) && $data['lpcms'] ? $data['lpcms'] : '';
		if (empty($file)) {
	...
	public function thumbAction() {
		...
		$file	= isset($data['lpcms']) && $data['lpcms'] && is_file($data['lpcms']) ? $data['lpcms'] : EXTENSION_PATH . '/null.jpg';
	...
	public function dataAction() {
	    ...
	    ob_start();
		if (count($data) == 1 && isset($data['lpcms_html_to_data'])) {
			$this->view->assign('data', $data['lpcms_html_to_data']);
		} else {
